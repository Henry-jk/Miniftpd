...
int tcp_client(const unsigned short port)
{
	...
	if(port>0)
	{
		...
		//char ip[16]={0};
		//getlocalip(ip);
		localaddr.sin_addr.s_addr=inet_addr("192.168.76.6");
		int on=1;
...
int tcp_server(const char *host,unsigned short port)
{
	...
	if(host!=NULL)
	{
		...
		servaddr.sin_addr.s_addr=inet_addr("192.168.76.6");
...
int getlocalip(char *ip) //获取本地ip地址的函数
{
	...
	strcpy(ip, inet_ntoa(*(struct in_addr*)hp->h_addr));//"192.168.76.6"  存在ip中
